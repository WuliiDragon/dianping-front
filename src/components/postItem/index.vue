<template>
  <ul class="item-wrap" :class="{padding: isHome}">
    ...
    <template >
      <li class="item-box" v-for="(item, index) in list" :key="index" @click="$router.push({name: 'postdetail', params: {post_id: item.post_id}})">

        <div class="intr-box">
          <div class="top-box">
            <h3 class="name">{{item.post_title}}</h3>
            <div v-if="userInfo.permission" class="distance">
              <Button type="error" shape="circle" icon="ios-trash" @click.stop="delete_post(item.post_id)"></Button>

            </div>
          </div>

          <div class="top-box">
            <h3 class="name">{{item.post_time}}</h3>
            <div class="distance">{{item.comment_num}} 条评论</div>
          </div>
        </div>
      </li>
    </template>
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          try {
            ...
            this.$http.post('http://127.0.0.1:5000/post/deletePost', data, {
