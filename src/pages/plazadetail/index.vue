...
export default {
  ...
  methods: {
    to_like(comment_id,index) {
      var self = this;
      this.data.plaza_comment[index].click_like = !this.data.plaza_comment[index].click_like
      // console.log(this.data.canteen_comment[index].click_like)
      this.flags = !this.flags


      if (this.data.plaza_comment[index].click_like){
        ...
        this.$http.post('http://127.0.0.1:5000/comment/like', data, {
          ...
          self.data.plaza_comment[index].comment_like = response.comment_like
