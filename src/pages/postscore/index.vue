...
export default {
  ...
  methods: {
    back() {
      ...
      this.$router.push({ name: 'postdetail', params: { post_id: this.post_id } });
    },
    insert() {
      try {
        var pic_ids=""
        ...
        this.$http.post('http://127.0.0.1:5000/comment/insertComment',data, {
          ...
          if(response.msg=='评论插入成功'){
              self.$router.push({ name: 'postdetail', params: { post_id: self.post_id } });
          }
